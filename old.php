<?php
			...
			function vote(){
				global $connect, $stateConnect, $_SESSION;
				...
				$statement->bind_result($userVoteResults);
				
				if($statement->fetch()){
					$userVotes = $userVoteResults;
				}
				echo "User Votes: " . $userVotes . "<br>";
				...
				$userVotes = implode(',', $voteItems);
				
				$statement->bind_param('is', $userVotes, $_SESSION['email']);
				
				$statement->execute();
				echo $_SESSION['email'] . " | " . $userVotes;//LOOK HERE
					...
					while ($row = mysqli_fetch_array($query)) {
						...
						if(is_array($userVotes) === true){
							if(in_array($items[$itemCounter]['name'], $userVotes) === true){
								...
								var_dump(in_array($items[$itemCounter]['name'], $userVotes) === true);
